...
import { PlanResult, GeneratedFile, BuildPhase, BuildTask } from '../../temporal/types.js';
...
- Tailwind CSS`;
  ...
  protected async execute(input: Record<string, any>): Promise<GeneratedFile[]> {
    const plan = input.plan as PlanResult;
  ...
  private async executePhase(phase: BuildPhase, projectName: string): Promise<GeneratedFile[]> {
    const files: GeneratedFile[] = [];
  ...
  private async executeTask(task: BuildTask, projectName: string): Promise<GeneratedFile | null> {
  ...
  private generateFileContent(filePath: string, projectName: string): string {
  ...
  private getFileType(filePath: string): 'source' | 'config' | 'asset' | 'doc' {
...
`;
  ...
  private generateHomePage(projectName: string): string {
...
`;
  ...
  private generatePage(projectName: string, pagePath: string): string {
...
export async function runCoordinatorAgent(
  ...
  return result as GeneratedFile[];
}
