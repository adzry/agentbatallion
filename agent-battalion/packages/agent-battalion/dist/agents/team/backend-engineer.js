...
- Secure sensitive endpoints`,
     ...
     */
    async generateBackend(requirements, dataModels, architecture) {
    ...
    async executeTask(task) {
        switch (task.title) {
            case 'generate_backend':
                ...
                return await this.generateBackend(reqs, models, arch[0]);
            default:
                throw new Error(`Unknown task: ${task.title}`);
        }
    }
    generatePrismaSchema(dataModels) {
...
`;
    ...
    generateApiRoutes(dataModels, requirements) {
        const routes = [];
        // Health route
        routes.push({
...
`,
        ...
        const needsAuth = requirements.some(r => r.description.toLowerCase().includes('auth') ||
...
`;
    ...
    generateMiddleware(requirements) {
        const middleware = [];
        // Auth middleware
        middleware.push({
            path: 'lib/auth.ts',
