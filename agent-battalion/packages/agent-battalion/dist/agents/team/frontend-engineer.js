...
exports.FrontendEngineerAgent = void 0;
const base_team_agent_js_1 = require("../base-team-agent.js");
class FrontendEngineerAgent extends base_team_agent_js_1.BaseTeamAgent {
...
- Write self-documenting code`,
     ...
     */
    async generateCode(architecture, designSystem, techStack, fileStructure) {
        ...
        for (const filePath of fileStructure) {
            ...
            if (content) {
                ...
                files.push(file);
                // Create artifact
                this.createArtifact('code', filePath.split('/').pop() || filePath, content, filePath);
            }
            completedFiles++;
     ...
     */
    async generateWithAI(filePath, architecture, designSystem, techStack) {
...
Return ONLY the TypeScript/TSX code, no markdown code blocks or explanations.`;
    ...
    async executeTask(task) {
        switch (task.title) {
            case 'generate_code':
                ...
                return await this.generateCode(arch[0], design[0], stack[0], structure[0]);
    ...
    generateFileContent(filePath, architecture, designSystem, techStack) {
        ...
        if (fileName === '.env.local')
            return this.generateEnvLocal();
        if (fileName === '.gitignore')
            return this.generateGitignore();
        // App structure
        if (filePath === 'app/layout.tsx')
            return this.generateLayout(designSystem);
        if (filePath === 'app/page.tsx')
            return this.generateHomePage(architecture, designSystem);
        if (filePath === 'app/loading.tsx')
            return this.generateLoadingPage();
        if (filePath === 'app/error.tsx')
        ...
        if (filePath === 'lib/constants.ts')
            return this.generateConstants();
        // Types
        if (filePath === 'types/index.ts')
            return this.generateTypes();
        // Hooks
        if (filePath.includes('hooks/'))
            return this.generateHook(filePath);
        // API routes
        if (filePath.includes('app/api/'))
            return this.generateApiRoute(filePath);
        // Docs
        if (fileName === 'README.md')
            return this.generateReadme(techStack);
        return null;
    }
...
`;
    ...
    generateHomePage(architecture, designSystem) {
        const projectName = this.projectContext?.name || 'My App';
...
`;
    ...
    generateComponent(filePath, architecture, designSystem) {
        const fileName = filePath.split('/').pop()?.replace('.tsx', '') || '';
        const componentName = this.toPascalCase(fileName);
        // Check component type based on path
        if (filePath.includes('layout/header'))
            return this.generateHeader();
        if (filePath.includes('layout/footer'))
            return this.generateFooter();
        if (filePath.includes('layout/navigation'))
            return this.generateNavigation();
        if (filePath.includes('sections/hero'))
            return this.generateHeroSection();
        if (filePath.includes('sections/features'))
            return this.generateFeaturesSection();
        if (filePath.includes('ui/button'))
            return this.generateButtonComponent();
        if (filePath.includes('ui/input'))
            return this.generateInputComponent();
        if (filePath.includes('ui/card'))
            return this.generateCardComponent();
        if (filePath.includes('ui/modal'))
            return this.generateModalComponent();
        // Generic component
        return `'use client';
...
exports.FrontendEngineerAgent = FrontendEngineerAgent;
